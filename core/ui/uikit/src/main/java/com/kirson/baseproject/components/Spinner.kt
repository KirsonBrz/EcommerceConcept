...
fun Spinner(
  ...
  Box(modifier = modifier.wrapContentSize(Alignment.TopStart)) {
    ...
    DropdownMenu(
      ...
      modifier = dropDownModifier.background(color = BaseProjectAppTheme.colors.backgroundSecondary)
    ) {
      options.forEachIndexed { index, element ->
        DropdownMenuItem(
          modifier = Modifier.background(color = BaseProjectAppTheme.colors.backgroundSecondary),
          onClick = {

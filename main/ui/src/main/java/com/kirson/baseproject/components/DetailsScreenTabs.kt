...
fun DetailsScreenTabs(
    tabs: List<String>, pagerState: PagerState, phoneDetails: APIPhoneDetails
) {
    val scope = rememberCoroutineScope()

    TabRow(selectedTabIndex = pagerState.currentPage,
        backgroundColor = Color.White,
        contentColor = BaseProjectAppTheme.colors.mainColor,
...
@Composable
fun TabShopScreen(phoneDetails: APIPhoneDetails) {
...
private fun Color.Companion.parse(colorText: String): Color =
...
fun CapacitySwitchChip(
    ...
    val color by animateColorAsState(
        targetValue = if (selected) BaseProjectAppTheme.colors.mainColor else Color.White,
        animationSpec = tween(durationMillis = 250)
    )
    Surface(
        modifier = Modifier
...
fun DetailsScreenTabsPreview() {
    BaseProjectAppTheme {
        ...
        DetailsScreenTabs(
            tabs = tabs, pagerState = pagerState, phoneDetails = APIPhoneDetails(
